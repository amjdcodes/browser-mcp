...
- The default window is Chromium's own `780×437`. Do NOT add `--window-size` to a desktop size: a 1440×900 default made every viewport capture ~5× more expensive under software rendering (over 30s per shot) and timed out the suite. Call `browser_resize` when a desktop layout is needed.
- Screenshots write to `./screenshots/` (override with `OUTPUT_DIR` env var). Paths are validated against traversal. `delay_ms` (0–5000) delays capture for animated pages. `filename` gets its extension from `format`: a matching one is kept (`.jpg`/`.jpeg` for jpeg), any other image extension is replaced, and a name without one is appended to.
- Full-page screenshots (`full_page: true`) scroll the whole page first (triggering IntersectionObserver/lazy rendering), then capture with a clip spanning the document and `captureBeyondViewport: true`. The viewport is deliberately **not** resized to the page height — doing so recomputed the layout against a fake viewport (100vh inflated, `position: fixed` elements smeared across the image), which broke RTL pages. Oversized pages are downscaled via `clip.scale`, never cropped.
...
- A crash restart spawns a new Chromium **with the same profile** (stale `SingletonLock`/`DevToolsActivePort` are cleared first), so `localStorage` and session state survive; the page itself returns to `about:blank`. The first successful tool result afterwards carries an extra text content item `{ sessionReset: true, reason }`.
...
- `browser_scroll` scrolls by direction / to element / to absolute coordinates, with `behavior: 'instant'` (no animation). A single axis may be given (`{y: 600}` keeps the current horizontal offset). Element mode reports `inViewport` (any part visible) and `fullyInViewport` (all of it fits). Every scroll runner waits ~100ms + double-rAF (compositor settle) before returning, so a screenshot in the next tool call is not a blank frame.
- `browser_resize` applies a persistent viewport override via `Emulation.setDeviceMetricsOverride` (presets: mobile `390×844 @3`, tablet `768×1024 @2`, desktop `1280×800 @1`, or explicit `width`+`height`, or `reset`). The override is stored on `Browser.viewport`, survives idle shutdown, and is re-applied automatically after a restart/reconnect. The response echoes the requested values and adds `measured` (`innerWidth`/`innerHeight`/`devicePixelRatio`/`scrollX`/`scrollWidth`) plus a `warning` when the page does not accept the requested width. A full-page screenshot no longer touches the viewport, so there is nothing to restore afterwards.
- `browser_evaluate` is **disabled by default** and refused with `[EVAL_DISABLED]` before the browser starts unless `ENABLE_EVAL_JS=1`; the refusal message explains the SSRF/cookie exposure and how to enable it, and `browser_snapshot` advertises the gate as `evaluateEnabled`. Results are serialized in-page by the static `IN_PAGE.serializeValue` (cycles handled; DOM/Function/Date/Error/Map/Set/BigInt readable) and truncated to `MAX_EVAL_LENGTH`. The remote object handle is always released.
- `browser_get_url` is a read-only tool returning `{ url, title, readyState }` — the only way to ask where the page currently is (e.g. after a click that navigated).
